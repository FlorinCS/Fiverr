...
const calendarDays = 24;
 ...
 function setCookie() {
    var arrCalendar = ['foo'];
    var json_str_calendar = JSON.stringify(arrCalendar);
...
const createCalendar = () => {
    for(let i = 0; i  < calendarDays; i++) {
        ...
        if(i<actualDay){
            if(JSON.parse(document.cookie).includes(calendarDoorText.innerText)){
...
function myFunction(){
    ...
    if(JSON.parse(document.cookie).includes(calendarDoorText.innerText)==false){
    let calendarCookie=JSON.parse(document.cookie);
 console.log(calendarCookie.push(calendarDoorText.innerText));
 document.cookie=JSON.stringify(calendarCookie);
    }
}
}
    }
